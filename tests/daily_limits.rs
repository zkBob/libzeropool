...
fn test_daily_limits_14() {
    ...
    deposit(&mut state, &mut rng, 100, 300, 0, true);
    transfer(&mut state, &mut rng, 100, 300, 0, true);
    withdrawal(&mut state, &mut rng, 100, 300, 0, true);

    deposit(&mut state, &mut rng, 100, 300, 1, true);
    transfer(&mut state, &mut rng, 100, 300, 1, true);
    withdrawal(&mut state, &mut rng, 100, 300, 1, true);

    deposit(&mut state, &mut rng, 100, 300, 2, true);
    transfer(&mut state, &mut rng, 100, 300, 2, true);
    withdrawal(&mut state, &mut rng, 101, 300, 2, false);
}

fn deposit<R: Rng>(state: &mut State<PoolBN256>, rng: &mut R, amount: u64, daily_limit: u64, day: u64, success: bool) {
    let (public, secret) = state.sample_deterministic_deposit(rng, &*POOL_PARAMS, amount as u64, day as u64, daily_limit as u64);
...
fn transfer<R: Rng>(state: &mut State<PoolBN256>, rng: &mut R, amount: u64, daily_limit: u64, day: u64, success: bool) {
    let (public, secret) = state.sample_deterministic_transfer(rng, &*POOL_PARAMS, amount as u64, day as u64, daily_limit as u64);
    let params = POOL_PARAMS.clone();
    let result = panic::catch_unwind(move || {
        let ref cs = DebugCS::rc_new();
        let ref p = CTransferPub::alloc(cs, Some(&public));
...
fn withdrawal<R: Rng>(state: &mut State<PoolBN256>, rng: &mut R, amount: u64, daily_limit: u64, day: u64, success: bool) {
    let (public, secret) = state.sample_deterministic_withdrawal(rng, &*POOL_PARAMS, amount as u64, day as u64, daily_limit as u64);
    let params = POOL_PARAMS.clone();
    let result = panic::catch_unwind(move || {
        let ref cs = DebugCS::rc_new();
        let ref p = CTransferPub::alloc(cs, Some(&public));
