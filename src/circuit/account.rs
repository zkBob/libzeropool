...
pub struct CAccount<C:CS> {
    ...
    pub daily_turnover: CBoundedNum<C, { constants::TURNOVER_SIZE_BITS }>,
}
...
impl<C:CS> CAccount<C> {
    pub fn hash<P: PoolParams<Fr = C::Fr>>(&self, params: &P) -> CNum<C> {
        let inputs = [
            ...
            self.daily_turnover.as_num().clone(),   
        ];
    ...
    pub fn is_initial(&self, poolid:&CNum<C>) -> CBool<C> {
        (self.i.as_num() + self.b.as_num() + self.e.as_num() + self.last_action_day.as_num() + self.daily_turnover.as_num()).is_zero() & 
